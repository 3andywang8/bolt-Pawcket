...
import { processShelterLocations, ShelterLocation } from '../../utils/geocoding';
...
export default function AdoptScreen() {
  ...
  const [allShelterLocations, setAllShelterLocations] = useState<ShelterLocation[]>([]);
  const [mapRegion, setMapRegion] = useState({
    latitude: 25.0330, // 台灣台北預設座標
    longitude: 121.5654,
    latitudeDelta: 0.5,
    longitudeDelta: 0.5,
  });
  ...
  // 篩選動物邏輯
  const filterAnimals = useCallback((animals: any[]) => {
    return animals.filter(animal => {
      // 動物類型篩選
      if (filters.animalType !== 'all') {
        if (filters.animalType === 'cat' && animal.type !== 'cat') return false;
        if (filters.animalType === 'dog' && animal.type !== 'dog') return false;
      }

      // 年齡篩選
      if (filters.ageGroup !== 'all') {
        if (filters.ageGroup === 'young' && !animal.age?.includes('幼')) return false;
        if (filters.ageGroup === 'adult' && !animal.age?.includes('成')) return false;
        if (filters.ageGroup === 'senior' && !animal.age?.includes('老')) return false;
      }
      ...
      if (filters.size !== 'all') {
        ...
        if (filters.size === 'medium' && (weight <= 10 || weight > 25)) return false;
  ...
  // 當篩選條件改變時更新顯示的收容所
  useEffect(() => {
    const filteredShelters = allShelterLocations.map(shelter => ({
      ...shelter,
        ...
        <View style={styles.headerButtons}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => setMapRegion({
              ...mapRegion,
              ...(userLocation || { latitude: 25.0330, longitude: 121.5654 }),
              latitudeDelta: 0.1,
              longitudeDelta: 0.1,
            })}>
